...
### 脏读
	当一个事务正在访问数据,并且对数据进行了修改，而这种数据还没有提交到数据库中,这时,另外一个事务也访问这个数据，然后使用了这个数据。因为这个数据还没有提交那么另外一个事务读取到的这个数据我们称之为脏数据。依据脏数据所做的操作肯定是不正确的。
### 不可重复读
	对于两个事务T1,T2。 T1从一个表中读取了一个字段，然后T2更新了该字段。之后，T1再次读取到同一个字段，值还是T1d
### 幻象读
...
| READ       COMMITED       （读已提交数据）      | ※只允许T1事务读取T2事务提交的变更，只可以避免脏读            |
